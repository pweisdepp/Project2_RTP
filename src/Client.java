...
// This class implements the client and the user interface which you use to send RTSP commands
// and which is used to display the video. Below is what the interface looks like. You will need to
// implement the actions that are taken when the buttons are pressed.

// Your first task is to implement the RTSP on the client side. To do this, you need to complete the
// functions that are called when the user clicks on the buttons in the user interface. For each button
// in the interface there is a handler function in the code. You will need to implement the following
// actions in each handler function.
//
// When the client starts, it also opens the RTSP socket to the server. Use this socket for sending
// all RTSP requests.
...
public class Client{
    ...
    final static String RTSPversion = "RTSP/1.0";
